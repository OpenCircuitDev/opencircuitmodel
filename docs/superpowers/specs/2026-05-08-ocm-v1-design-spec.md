# OpenCircuitModel (OCM) — Design Spec v0.7

> **Status:** v1 implementation mostly built (Phases 1-8 on main as of 2026-05-09); six research-driven revisions; implementation plan in `../plans/2026-05-08-ocm-v1-implementation-plan.md`
> **Date:** 2026-05-08 (v0.2) → 2026-05-09 (v0.3) memory deep dive → 2026-05-09 (v0.4) Memory Palace → 2026-05-09 (v0.5) VM/cloud policy → 2026-05-09 (v0.6) post-build hygiene + bench INACTIVE convention → 2026-05-09 (v0.7) mobile compute policy
...
| v1.5 | Mobile consumer client (iOS + Android, no compute contribution) | ❌ not started — design locked v0.7 row 33 |
| v2 | Real iroh / libp2p mesh transport | ⚠️ trait scaffold only |
| v2.5 | Mobile opportunistic contributor — iPad M-series first | ❌ not started — design locked v0.7 row 33 |
| v3 | Reciprocity ledger | ❌ not started |
| v3.5 | Mobile opportunistic contributor — phones (charging + WiFi + foreground) | ❌ not started — design locked v0.7 row 33 |
| v4 | Codesigning + auto-update (incl. App Store distribution path for mobile) | ❌ not started |
...
## v0.7 revisions (2026-05-09)

One change (row 33) plus targeted revisions to rows 6, 8, 13, 29:

1. **Mobile compute policy locked — opportunistic contributor with staged rollout** (decision row 33 below). Mobile devices participate in the OCM mesh in two distinct modes: (a) **consumer** — using mesh compute from a phone or tablet via the existing OpenAI-compat HTTP API, default-on; ships in v1.5 as an iOS + Android client, distributed via TestFlight initially with App Store distribution gated on v4 codesigning. (b) **opportunistic contributor** — donating compute back to the mesh, default-OFF, gated on (charging AND WiFi AND foreground) OR an explicit user "donate compute now" press. Tablet contributor mode (v2.5) precedes phone contributor mode (v3.5) because M-series iPads handle sustained inference without battery anxiety in a way phones do not. Honest limit: per-phone contribution measures in seconds-per-day under realistic usage; the value to the mesh is aggregate-at-scale, not per-device. Honest non-recommendation: never pursue full off-foreground / off-charger phone participation — battery economics + iOS sandboxing both fight you. Scoped revisions to existing rows: row 6 extends Apple Silicon scope to include iPad M-series; row 8 names iroh's Swift + Kotlin (uniffi) bindings as the mobile transport path with libp2p-mobile as escape hatch; row 13 expands the platform list with consumer/contributor segmentation called out; row 29 maps Zone A encryption to iOS Keychain + Android Keystore. Full reasoning to be written into `../research/2026-05-10-mobile-compute-mesh-analysis.md`.
...
| 6 | Inference engine — Apple Silicon peers (incl. iPad M-series via v0.7 mobile policy) | **llama.cpp + Metal default; MLX-LM opt-in for advanced users; MLC-LLM for cross-platform mobile (iOS / Android / iPadOS) when mobile contributor mode lands v2.5+** | llama.cpp most stable on macOS; MLX-LM 20-40% faster autoregressive but batching not yet vLLM-class production-mature; MLC-LLM picked for mobile because of its Vulkan/Metal/OpenCL portability across iOS + Android + iPadOS without per-platform forks. Apple Silicon scope extended to iPad M-series per v0.7 row 33 — same Metal path as Mac, same Q4 model lineup |
...
| 8 | Mesh transport (v2+, mobile bindings v1.5+) | **iroh primary, libp2p as escape hatch via libp2p-iroh bridge. Mobile bindings: iroh's official Swift + Kotlin (uniffi-generated) for iOS / Android / iPadOS; libp2p-mobile reserved as escape hatch.** | libp2p residential NAT success 70% (measured 4.4M attempts); iroh ~90% (vendor-claimed, plausibly 80%+); iroh single-vendor risk mitigated by libp2p-iroh bridge keeping app protocols transport-agnostic. Per v0.7 row 33: mobile clients ship in v1.5 as consumers (no iroh wire participation needed — they connect to a desktop OCM over LAN via /v1/* HTTP API). Iroh Swift/Kotlin bindings activate in v2.5+ when mobile contributor mode opens, beginning with iPad |
...
| 13 | Platforms | **Desktop v1: macOS + Windows + Linux. Mobile v1.5+ (consumer): iOS + Android + iPadOS via Capacitor / Tauri 2 mobile shell wrapping the SvelteKit frontend. Mobile v2.5+ (contributor): iPadOS first, iOS + Android phones in v3.5+ with conservative defaults.** | Tauri makes desktop cheap. Mobile consumer is also cheap because it reuses the SvelteKit frontend — only the shell + native bridge change. Per v0.7 row 33: distribution paths are TestFlight + sideload + F-Droid initially, full App Store after v4 codesigning. Consumer-vs-contributor split lets v1.5 ship without battery / thermal / background-execution complexity |
...
| 29 | **Encryption mapped onto privacy zones A/B/C (new in v0.4, mobile keystore mapping added v0.7)** | **Zone A (local Mem0): SQLCipher AES-256 with Argon2id-derived key from user passphrase (~5-15% latency overhead). Zone B (personal palace): age symmetric encryption + Ed25519 signing for at-rest; user identity keypair stored encrypted in OS secrets store. Zone C (mesh-published): Ed25519 signatures always; age recipient encryption when private group share (`mesh: group@<id>` frontmatter). Transport encryption already provided by iroh QUIC TLS 1.3. OS secrets store mapping: macOS Keychain / Windows DPAPI / Linux libsecret on desktop; **iOS Keychain Services / Android Keystore on mobile per v0.7 row 33** (both hardware-backed where Secure Enclave / StrongBox is available — actually a SECURITY UPGRADE over typical desktop libsecret on Linux). Honest limit: v6+ sharded inference is NOT privacy-preserving against malicious borrowed-machine operator (TEE hardware-gated; FHE decade-out)** | Sandbox I measures Mem0 encryption overhead. age chosen over GPG (5K LOC vs 250K, vastly easier to audit). Ed25519 signatures = ~50-100µs/op; age = ~50 MB/s; cryptographic overhead is sub-millisecond on KB-MB nodes — doesn't violate quick-look-up budgets. Mobile keystores activate in v1.5 even for consumer-only mode because the user identity keypair (used for personal palace + future contributor signatures) lives there from day one. Full mapping in encryption-compression research note |
...
| 33 | **Mobile compute policy — opportunistic contributor with staged rollout (new in v0.7)** | **Two participation modes for mobile: (a) CONSUMER — phone/tablet uses mesh compute via the existing /v1/* HTTP API, default-on, ships in v1.5 as iOS + Android client (Capacitor or Tauri 2 mobile webview reusing the SvelteKit frontend); TestFlight initially, App Store after v4 codesigning. (b) OPPORTUNISTIC CONTRIBUTOR — phone/tablet donates compute back, default-OFF, gated on (charging AND WiFi AND foreground) OR explicit "donate now" press. Tablet contributor mode lands v2.5 (M-series iPads handle sustained inference without battery anxiety); phone contributor mode lands v3.5 with conservative defaults. Three explicit non-goals: (i) NEVER full off-foreground / off-charger phone participation — battery economics + iOS sandboxing both fight you; (ii) cellular-data contributions remain off by default, opt-in only with metered-data warning; (iii) no privileged compute path for "Apple Intelligence" or system-level NPU APIs in v1.x — that's an App Store-policy minefield. Per-device contribution measures in seconds-per-day under realistic phone usage; the value to the mesh is aggregate-at-scale, not per-device. iPad M-series in v2.5 is where mobile contribution becomes meaningful in absolute terms.** | Names the policy that's been implicit/aspirational since v0.1. Distinguishes consumer vs contributor cleanly so the v1.5 ship can be small (consumer-only iOS + Android client; reuses existing daemon as the desktop "server" you connect to from mobile). Tablet-first contributor staging avoids the battery-anxiety trap that would have hurt phone contributor adoption. Honest about App Store realities: "OCM as marketplace for compute" is gray-area on iOS; TestFlight + sideload (and F-Droid for Android) are the reliable v1.5-v3 paths until v4 codesigning + Apple Developer verification land. Full reasoning in `../research/2026-05-10-mobile-compute-mesh-analysis.md` (to be written) |
