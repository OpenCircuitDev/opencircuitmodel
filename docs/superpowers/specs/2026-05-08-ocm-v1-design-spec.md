...
OCM now has three structurally-aligned network-effect levers (v2+ inference sharing → v2+ skill federation → v3.5+ memory palace federation), all governed by the Effective-Context Triad. Each operates on a different layer (hardware → programs → knowledge); each is something no closed-cloud system can match structurally.
