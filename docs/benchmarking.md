...
### Snapshot views: dashboard + coverage

Two commands answer "what is the current state?" without re-running anything:

```bash
# All ACTIVE sandboxes, latest verdict per (hypothesis, hardware) pair.
# Prints a markdown table to stdout; --write commits to docs/metrics.md.
bench dashboard --write docs/metrics.md

# Same, but exits 1 if any sandbox is REFUTED, INCONCLUSIVE, or has no run.
# CI uses this to gate merges that would weaken the published numbers.
bench dashboard --check

# Map spec rows to validating sandboxes (one row per locked decision).
# Surfaces "unvalidated rows" (no sandbox) and "orphan sandboxes" (no row).
bench coverage --write docs/coverage.md
```

`docs/metrics.md` and `docs/coverage.md` are checked into the repo and regenerated by CI. If you change a sandbox or flip its state, run both commands locally and commit the regenerated files — CI will fail the PR otherwise.

### Trend tracking

Every `bench run` appends one line to `bench/results/history.jsonl` capturing the run's metric, verdict, git SHA, wall-clock time, and repeats. This is the single fast scan path for plotting trends or bisecting regressions without re-parsing every `summary.json`. The schema is `HistoryRecord` in [bench/bench/metrics.py](../bench/bench/metrics.py).
